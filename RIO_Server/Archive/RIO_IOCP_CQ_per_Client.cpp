...
constexpr auto CQ_SIZE_FACTOR = 20;
...
struct SOCKETINFO
{
	...
	WSAOVERLAPPED				rio_notify_ov;
	RIO_NOTIFICATION_COMPLETION rio_noti_cp;
};
...
// 하나의 CQ로 모두 처리
RIO_CQ g_rio_cq[MAX_USER];
mutex g_rio_cq_lock[MAX_USER];
...
void do_worker()
{
	while (true) {
		...
		UINT clientid = static_cast<int>(key64);

		// 완료통지 후 cq에 쌓인 result 처리
		RIORESULT results[MAX_RIO_RESULTS];
		//g_rio_cq_lock[clientid].lock();
		ULONG numResults = gRIO.RIODequeueCompletion(g_rio_cq[clientid], results, MAX_RIO_RESULTS);
		//g_rio_cq_lock[clientid].unlock();
		...
		gRIO.RIONotify(g_rio_cq[clientid]);
	}
}
...
int main()
{
	...
	//WSAOVERLAPPED iocp_over[MAX_USER];
	//RIO_NOTIFICATION_COMPLETION rio_noti[MAX_USER];
	...
	//	rio_noti[i].Iocp.CompletionKey = NULL;
	...
	while (true) {
		...
		// SocketInfo가 rio notify desc를 가지도록
		// 각 Client가 User id를 idx로 CQ를 하나씩 사용
		ZeroMemory(&new_player->rio_notify_ov, sizeof(WSAOVERLAPPED));
		new_player->rio_noti_cp.Type = RIO_IOCP_COMPLETION;
		new_player->rio_noti_cp.Iocp.IocpHandle = g_iocp;
		new_player->rio_noti_cp.Iocp.Overlapped = &new_player->rio_notify_ov;
		new_player->rio_noti_cp.Iocp.CompletionKey = (void*)new_player->id;
		g_rio_cq[new_player->id] = gRIO.RIOCreateCompletionQueue(MAX_CQ_SIZE_PER_RIO_THREAD, &new_player->rio_noti_cp);
		...
		new_player->req_queue = gRIO.RIOCreateRequestQueue(
			...
			g_rio_cq[new_player->id], g_rio_cq[new_player->id],
		...
		gRIO.RIONotify(g_rio_cq[new_player->id]);
	}
